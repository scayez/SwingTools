...
class SwingData:
    def __init__(self, 
        ...
        self.filenumber=self.extract_number()
    ...
    def extract_number(self) -> int:
        ...
        filename = self.filename.split('\\')[-1]  # Get the last part of the path
    ...
    def save_integration(self):
            ...
            if q.ndim == 2 and i.ndim == 2:  # Multiple frames
                for frame in range(self.nb_frames):
                    filename = os.path.join(
                        ...
                        f"{self.filenumber:05d}{self.filename_suffix}_frame{frame + 1:03d}_sector_{angle}_{delta_angle}.txt"
                    )
                    np.savetxt(filename, np.column_stack((q[frame], i[frame])), header="q, i")
            else:  # Single frame (mean image)
                filename = os.path.join(
                    self.integration_folder, 
                    f"{self.filenumber:05d}{self.filename_suffix}_sector_{angle}_{delta_angle}.txt"
                )
        ...
        if self.cavingtag:
            ...
            for frame in range(self.nb_frames):
                filenameq = os.path.join(outdir,f"{self.filenumber}{self.filename_suffix}_frame{frame + 1:03d}_q.txt")
                filenamechi = os.path.join(outdir,f"{self.filenumber}{self.filename_suffix}_frame{frame + 1:03d}_chi.txt")
                filenamei = os.path.join(outdir,f"{self.filenumber}{self.filename_suffix}_frame{frame + 1:03d}_i.txt")
    ...
    def plot_data(self):
        ...
        for frame_idx in range(self.nb_frames):
            filename = os.path.join(
                    plots_folder,
                    f"{self.filenumber:05d}{self.filename_suffix}_frame{frame_idx + 1:03d}.png"
                )
    ...
    def _create_plot(self,integration_results,frame_idx,plots_folder,output_filename):
        ...
        base_name = f"{self.filenumber:05d}"
        suffix = "_norm" if self.normalized else "_raw"
                
        if frame_idx:
            frame_info = f"frame{frame_idx:03d}"
            default_name = f"{self.filenumber:05d}_{frame_info}_{suffix}.png"
        else:
            default_name = f"{self.filenumber:05d}_{suffix}.png"
