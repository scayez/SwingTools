...
class SwingData:
    def __init__(self, 
        ...
        self.sectors = sectors if sectors is not None else None  # Default value for sectors
    ...
    def extract_from_h5(self):
        ...
        with h5py.File(self.filename, "r") as f:
            ...
            self.reduce_name=reduce_filename.split('.')[0]
    ...
    def integrate(self):
        ...
        for angle, delta_angle in sectors:
            q_list = []
            i_list = []
    ...
    def save_integration(self):
        ...
        self.filename_suffix = "_norm" if self.normalized else "_raw"
            ...
            if q.ndim == 2 and i.ndim == 2:  # Multiple frames
                for frame in range(self.nb_frames):
                    filename = os.path.join(
                        ...
                        f"{self.reduce_name}_{self.sample_name}{self.filename_suffix}_img{frame + 1:03d}_sect_{angle}_{delta_angle}.txt"
                    )
                    np.savetxt(filename, np.column_stack((q[frame], i[frame])), header="q, i")
            else:  # Single frame (mean image)
                filename = os.path.join(
                    self.integration_folder, 
                    f"{self.reduce_name}_{self.sample_name}{self.filename_suffix}_sector_{angle}_{delta_angle}.txt"
                )
        ...
        if self.cavingtag:
            ...
            for frame in range(self.nb_frames):
                filenameq = os.path.join(outdir,f"{self.reduce_name}_{self.sample_name}{self.filename_suffix}_frame{frame + 1:03d}_q.txt")
                filenamechi = os.path.join(outdir,f"{self.reduce_name}_{self.sample_name}{self.filename_suffix}_frame{frame + 1:03d}_chi.txt")
                filenamei = os.path.join(outdir,f"{self.reduce_name}_{self.sample_name}{self.filename_suffix}_frame{frame + 1:03d}_i.txt")
    ...
    def plot_data(self):
        ...
        for frame_idx in range(self.nb_frames):
            filename = os.path.join(
                    plots_folder,
                    f"{self.reduce_name}_{self.sample_name}{self.filename_suffix}_frame{frame_idx + 1:03d}.png"
                )
    ...
    def _create_plot(self,integration_results,frame_idx,plots_folder,output_filename):
        ...
        base_name = f"{self.reduce_name}_{self.sample_name}"
        suffix = "_norm" if self.normalized else "_raw"
                
        if frame_idx:
            frame_info = f"frame{frame_idx:03d}"
            default_name = f"{self.reduce_name}_{self.sample_name}_{frame_info}_{suffix}.png"
        else:
            default_name = f"{self.reduce_name}_{self.sample_name}_{suffix}.png"
        ...
        if self.cavingtag==False:
            ...
            # Overlay the chi map mask (displayed in gray) to visualize integration sectors
